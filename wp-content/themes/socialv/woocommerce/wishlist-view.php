...
<!-- WISHLIST TABLE -->
<table class="shop_table cart wishlist_table wishlist_view traditional responsive <?php $no_interactions ?  esc_attr_e('no-interactions', 'socialv') : ''; ?> <?php $enable_drag_n_drop ? esc_attr_e('sortable', 'socialv') : ''; ?> " data-pagination="<?php echo esc_attr($pagination); ?>" data-per-page="<?php echo esc_attr($per_page); ?>" data-page="<?php echo esc_attr($current_page); ?>" data-id="<?php echo esc_attr($wishlist_id); ?>" data-token="<?php echo esc_attr($wishlist_token); ?>">
	...
	<thead>
		<tr>
			...
			<th class="product-name">
				<span class="nobr">
					<?php echo esc_html(apply_filters('yith_wcwl_wishlist_view_name_heading', __('Product name', 'socialv'), $wishlist)); ?>
				</span>
			</th>
			...
			<?php if ($show_price || $show_price_variations) : ?>
				...
				<th class="product-price">
					<span class="nobr">
						<?php echo esc_html(apply_filters('yith_wcwl_wishlist_view_price_heading', __('Unit price', 'socialv'), $wishlist)); ?>
					</span>
				</th>
			<?php endif; ?>
			...
			<?php if ($show_quantity) : ?>
				...
				<th class="product-quantity">
					<span class="nobr">
						<?php echo esc_html(apply_filters('yith_wcwl_wishlist_view_quantity_heading', __('Quantity', 'socialv'), $wishlist)); ?>
					</span>
				</th>
			<?php endif; ?>
			...
			<?php if ($show_stock_status) : ?>
				...
				<th class="product-stock-status">
					<span class="nobr">
						<?php echo esc_html(apply_filters('yith_wcwl_wishlist_view_stock_heading', __('Stock status', 'socialv'), $wishlist)); ?>
					</span>
				</th>
			<?php endif; ?>
			...
			<?php if ($enable_drag_n_drop) : ?>
				...
				<th class="product-arrange">
					<span class="nobr">
						<?php echo esc_html(apply_filters('yith_wcwl_wishlist_view_arrange_heading', __('Arrange', 'socialv'), $wishlist)); ?>
					</span>
				</th>
			<?php endif; ?>
	...
	<tbody class="wishlist-items-wrapper">
		...
		?>
					<tr id="yith-wcwl-row-<?php echo esc_attr($item->get_product_id()); ?>" data-row-id="<?php echo esc_attr($item->get_product_id()); ?>">
						...
						<?php if ($show_remove_product) : ?>
							<td class="product-remove">
								<div>
									<a href="<?php echo esc_url($item->get_remove_url()); ?>" class="remove remove_from_wishlist" title="<?php echo esc_attr(apply_filters('yith_wcwl_remove_product_wishlist_message_title', __('Remove this product', 'socialv'))); ?>"><i class="iconly-Delete icli"></i></a>
								</div>
							</td>
						<?php endif; ?>
						...
						<td class="product-name">
							...
							<a class="socialv-product-title" href="<?php echo esc_url(get_permalink(apply_filters('woocommerce_in_cart_product', $item->get_product_id()))); ?>">
						...
						<?php if ($show_stock_status) : ?>
							<td class="product-stock-status">
								...
								<?php echo 'out-of-stock' === $stock_status ? '<span class="wishlist-out-of-stock">' . esc_html(apply_filters('yith_wcwl_out_of_stock_label', __('Out of stock', 'socialv'))) . '</span>' : '<span class="wishlist-in-stock">' . esc_html(apply_filters('yith_wcwl_in_stock_label', __('In Stock', 'socialv'))) . '</span>'; ?>
						...
						<?php if ($show_last_column) : ?>
							<td class="product-add-to-cart">
								...
								if ($show_dateadded && $item->get_date_added()) :
									...
									echo '<span class="dateadded">' . esc_html(sprintf(__('Added on: %s', 'socialv'), $item->get_date_added_formatted())) . '</span>';
								endif;
								?>
								...
								<?php if ($show_add_to_cart && isset($stock_status) && 'out-of-stock' !== $stock_status) : ?>
									<?php
									echo	socialv()->socialv_get_comment_btn($tag = "a",  $label =  esc_html__("View Product", "socialv"), $attr = array(
										'href' => esc_url($product->get_permalink()),
										'data-quantity' => esc_attr($show_quantity ? $item->get_quantity() : 1),
										'class' => ' btn-hover',
									)); ?>
								<?php endif ?>
								...
								<?php if ($move_to_another_wishlist && $available_multi_wishlist && count($users_wishlists) > 1) : ?>
									<?php if ('select' === $move_to_another_wishlist_type) : ?>
										<select class="change-wishlist selectBox">
											<option value=""><?php esc_html_e('Move', 'socialv'); ?></option>
											<?php
									...
									<?php else : ?>
										<a href="#move_to_another_wishlist" class="move-to-another-wishlist-button" data-rel="prettyPhoto[move_to_another_wishlist]">
											<?php echo esc_html(apply_filters('yith_wcwl_move_to_another_list_label', __('Move to another list &rsaquo;', 'socialv'))); ?>
										</a>
									<?php endif; ?>
								...
								<?php if ($repeat_remove_button) : ?>
									<a href="<?php echo esc_url($item->get_remove_url()); ?>" class="remove_from_wishlist button" title="<?php echo esc_attr(apply_filters('yith_wcwl_remove_product_wishlist_message_title', __('Remove this product', 'socialv'))); ?>"><?php esc_html_e('Remove', 'socialv'); ?></a>
								<?php endif; ?>
			...
			?>
			<tr>
				<td colspan="<?php echo esc_attr($column_count); ?>" class="wishlist-empty"><?php echo esc_html(apply_filters('yith_wcwl_no_product_to_remove_message', __('No products added to the wishlist', 'socialv'), $wishlist)); ?></td>
			</tr>
		<?php
		endif;
