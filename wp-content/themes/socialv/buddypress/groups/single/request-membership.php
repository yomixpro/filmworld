...
<?php if ( !bp_group_has_requested_membership() ) : ?>
	<h2 class="bp-screen-reader-text"><?php esc_html_e( 'Group membership request form', 'socialv' ); ?></h2>
	...
	<p>
		...
		echo esc_html(
			sprintf(
				...
				__( 'You are requesting to become a member of the group "%s".', 'socialv' ),
				bp_get_group_name()
			)
		);
		?>
	...
	<form action="<?php bp_group_form_action('request-membership' ); ?>" method="post" name="request-membership-form" id="request-membership-form" class="standard-form">
		<label for="group-request-membership-comments"><?php esc_html_e( 'Comments (optional)', 'socialv' ); ?></label>
		...
		<p><input type="submit" name="group-request-send" id="group-request-send" value="<?php echo esc_attr_x( 'Send Request', 'button', 'socialv' ); ?>" />
