...
$expanded .= $is_dark_mode == 0 ? " light-mode" : "";
...
<div class="redux-container <?php echo esc_attr($expanded); ?>">
	...
	<form method="post" action="<?php echo esc_attr($actionn); ?>" data-nonce="<?php echo esc_attr($nonce); ?>" enctype="multipart/form-data" class="redux-form-wrapper" data-opt-name="<?php echo esc_attr($this->parent->args['opt_name']); ?>">
		...
		<?php if (!empty($this->parent->options_class->no_panel)) { ?>
			<input type="hidden" name="<?php echo esc_attr($this->parent->args['opt_name']); ?>[redux-no_panel]" value="<?php echo esc_attr(implode('|', $this->parent->options_class->no_panel)); ?>" />
		<?php } ?>
