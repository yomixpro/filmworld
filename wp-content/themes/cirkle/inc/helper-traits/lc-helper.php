...
class Helper {
    ...
    public function theme_menu() { 
        add_theme_page( esc_html__( 'Theme License', 'artex' ), esc_html__( 'Theme License', 'artex' ), "manage_options", "rtlc", array( $this, 'create_admin_page' ), null, 99);
    }
    ...
    public function create_admin_page() {
        ...
        ?>
        <div class="wrap">
            <h1><?php esc_html_e( 'Theme License', 'artex' ); ?></h1>
            <form method="post" action="options.php">
            <?php
                // This prints out all hidden setting fields
                settings_fields( 'rt_option_group' );
                do_settings_sections( 'artex-setting' ); 
            ?>
            </form>
        </div>
        <?php
    ...
    public function theme_option() {        
        ...
        add_settings_field(
            'rt_purchase_code',  
            esc_html__( 'Purchase Code', 'artex' ),
        ...
        add_settings_field(
            'rt_license_status', 
            esc_html__( 'License Status', 'artex' ), 
    ...
    public function license_status_callback() {  
        ...
        $status_text = esc_html__( 'Not Activated', 'artex' );
         
        if ( rtlc_is_valid()['success'] ) {
            $verify = true;
            $status_text = esc_html__( 'Activated', 'artex' ); 
        } else if ( isset( rtlc_is_valid()['domain_match'] ) && !rtlc_is_valid()['domain_match'] ) {  
            $status_text = esc_html__( 'Domain Mismtach', 'artex' ); 
        }
    ...
    public function license_note_callback() { 
        $status = esc_html__( 'Please keep in mind, you can activate one license in one domain, if you face any problem in activation, please contact our', 'artex' ).' <a href="https://www.radiustheme.com/contact/">'.esc_html__( 'Support Center', 'artex' ).'</a>'; 
    ...
    public function license_check_callback() {
        printf(
            '<input type="button" id="rtlc_license_check" class="button button-primary rtcl-active-btn" value="%s" /> <span class="rtlc-loader"><i class="dashicons dashicons-update spin"></i></span>', esc_html__( 'Activate License', 'artex' )
        );
    }
