...
const Reactionable = (props) => {
    const sortUserReactions = (reactionData, order = 'DESC') => {
        ...
        app.deepExtend(reactions, reactionData);

        // console.log('REACTIONABLE - SORTED REACTION DATA: ', sortedReactionData);

        return reactions.sort((firstEl, secondEl) => {
    ...
    const createUserReaction = (reactionID) => {
        ...
        app.deepExtend(config, props.entityData);
...
const ActivityReactionable = (props) => {
    return Reactionable({
        ...props,
        createUserReaction: router.createActivityUserReaction,
        deleteUserReaction: router.deleteActivityUserReaction
    });
};
...
const PostReactionable = (props) => {
    return Reactionable({
        ...props,
        createUserReaction: router.createPostUserReaction,
        deleteUserReaction: router.deletePostUserReaction
    });
};
...
const updatePostReactions = (post, newReactions) => {
    const newPost = app.deepMerge(post);
...
const getAllReactionUsers = (reactionData) => {
    ...
    for (const reaction of reactionData) {
        ...
        for (const user of reaction.users) {
            const userReaction = app.deepMerge(user);
